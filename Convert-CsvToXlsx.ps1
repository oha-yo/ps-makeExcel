param(
    ...
    [Parameter()][bool]$QuotedFields = $true
)
...
$currentLineNumber++
$columnCount = if ($QuotedFields) {
    ([regex]::Matches($firstDataLine, '"').Count) / 2
} else {
    ($firstDataLine -split ',').Length
}
...
foreach ($line in $linesToProcess) {
    $columns = if ($QuotedFields) {
        [regex]::Matches($line, '"(.*?)"') | ForEach-Object { $_.Groups[1].Value }
    } else {
        $line -split ','
    }
